...
class ExerciseCacheVersionTest(SimpleTestCase):
    ...
    def test_recent_randomized_version_is_reused(self):
        cache.set(
        ...
        version = ExerciseCache.cached_exercise_version(
            123,
            "en",
            max_age=ExerciseCache.VERSION_MAX_AGE,
        )

        self.assertEqual(version, "version")

    def test_expired_nonrandomized_version_is_refreshed(self):
        cache.set(
            ExerciseCache._key(123, modifiers=["en"]),
            (time(), {"exercise_version": "old", "expires": time() - 1}),
        )
    ...
    def test_old_nonrandomized_version_is_refreshed_before_long_expiry(self):
        cache.set(
            ExerciseCache._key(123, modifiers=["en"]),
            (
                time() - ExerciseCache.VERSION_MAX_AGE - 1,
                {"exercise_version": "old", "expires": time() + 60 * 60},
            ),
        )
...
class FeedbackVersionTest(SimpleTestCase):
    def test_grader_response_version_is_stored_and_invalidates_old_page(self):
        ...
        def set_page_data(page, remote_page, parsed_exercise):
            page.is_loaded = True
