...
# Move to aplus-manual directory and build the course if it hasn't already been built yet
if ! [ -d `dirname "$0"`/aplus-manual/_build ]; then
    cd `dirname "$0"`/aplus-manual
    ./docker-compile.sh
    cd ..
fi

if [ -z "$APLUS_LOCAL_SETTINGS" ] && ! [ -f "aplus/local_settings.py" ]; then
    echo "Creating 'aplus/local_settings.py' from 'aplus/local_settings.example.py' since it does not exist yet..."
    cp aplus/local_settings.example.py aplus/local_settings.py
fi
...
python3 manage.py compilemessages --ignore=.venv --ignore=venv
